  ...
  return (
    <>
      <div className="flex items-center gap-2">
        ...
        {!user ? (
          <button onClick={handleLogin} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-gray-700 bg-white hover:bg-gray-50 border border-gray-200 rounded-md transition-colors">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={handleOpenModal} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-white bg-[#0A66C2] hover:bg-[#004182] rounded-md transition-colors shadow-xs">
              <Cloud size={13} /> Nuvem
            </button>
            <button onClick={handleLogout} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-md transition-colors" title="Sair da conta">
              <LogOut size={13} />
            </button>
          </div>
        )}
          ...
          <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[85vh] flex flex-col shadow-2xl border border-gray-100 overflow-hidden">
            ...
            <div className="p-6 overflow-y-auto flex-1">
              <div className="mb-6 flex flex-col items-center justify-center p-6 bg-blue-50/50 rounded-xl border border-blue-100/50">
                <p className="text-sm text-blue-800 mb-1 text-center opacity-80">Logado como <strong>{user?.email}</strong></p>
                {currentProjectId && (
                  <p className="text-xs text-blue-700 mb-3 text-center opacity-70">Projeto atual: <strong>{currentProjectName || settings.title || "Ebook"}</strong> · v{currentProjectVersion || 1}</p>
                )}
                <button onClick={() => saveToCloud(false)} disabled={isSyncing} className="flex items-center gap-2 px-6 py-2.5 bg-[#245C5A] text-white rounded-lg font-bold hover:bg-[#1b4342] shadow-sm disabled:opacity-50 transition-colors">
              ...
              <div className="flex items-center justify-between mb-4 px-1">
                ...
                <span className="text-[10px] font-bold text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">{cloudProjects.length} projeto{cloudProjects.length === 1 ? "" : "s"}</span>
              </div>
              <div className="space-y-3">
                {cloudProjects.length === 0 && <div className="text-center py-8 text-gray-400 italic">Nenhum projeto salvo na nuvem ainda.</div>}
                    ...
                    <div key={proj.id} className={`flex flex-col gap-3 bg-white border rounded-lg p-4 hover:border-[#245C5A] hover:shadow-sm transition-all group ${isCurrent ? "border-[#245C5A] bg-[#F4EFE7]/30" : "border-gray-200"}`}>
                      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                        <div className="min-w-0 flex-1">
                          {isEditing ? (
                            <div className="flex items-center gap-2">
                              <input value={editingProjectTitle} onChange={(e) => setEditingProjectTitle(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") saveProjectTitle(proj); if (e.key === "Escape") cancelEditingProjectTitle(); }} className="w-full border border-gray-300 rounded-md px-2 py-1 text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-[#245C5A]/30" autoFocus />
                              <button onClick={(e) => saveProjectTitle(proj, e)} className="p-1.5 rounded-md text-green-700 hover:bg-green-50" title="Salvar nome"><Check size={16} /></button>
                              <button onClick={cancelEditingProjectTitle} className="p-1.5 rounded-md text-gray-500 hover:bg-gray-100" title="Cancelar"><X size={16} /></button>
                            </div>
                          ) : (
                            <div className="flex items-center gap-2 min-w-0">
                              <h4 className="font-bold text-gray-900 truncate">{proj.title || "Ebook"}</h4>
                              {isCurrent && <span className="text-[9px] bg-[#245C5A] text-white px-1.5 py-0.5 rounded font-mono uppercase font-bold shrink-0">Atual</span>}
                            </div>
                          )}
                          <p className="text-xs text-gray-500 mt-1">v{proj.version || 1} · Atualizado em: {getProjectDateLabel(proj.updatedAt)}</p>
                        </div>
                        <div className="flex items-center gap-2 w-full sm:w-auto">
                          <button onClick={() => loadFromCloud(proj.id)} className="flex border border-gray-200 items-center gap-1.5 px-3 py-1.5 bg-gray-50 hover:bg-[#E6F4EA] hover:text-[#137333] hover:border-[#A3E5B7] text-gray-700 text-sm font-medium rounded-md transition-colors flex-1 sm:flex-none justify-center shrink-0">
                            <DownloadCloud size={16} /> Carregar
                          </button>
                          <button onClick={(e) => startEditingProjectTitle(proj, e)} className="p-2 border border-gray-200 rounded-md text-gray-500 hover:text-[#245C5A] hover:bg-[#F4EFE7] transition-colors" title="Editar nome do projeto"><Pencil size={15} /></button>
                          <button onClick={(e) => deleteCloudProject(proj, e)} className="p-2 border border-red-100 rounded-md text-red-500 hover:text-red-700 hover:bg-red-50 transition-colors" title="Excluir projeto da nuvem"><Trash2 size={15} /></button>
                        </div>
                      </div>
                    </div>
                  );
